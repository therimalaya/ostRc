...
test_that("Will remove observation from numerator and denominator if hp_type is missing.", {
  ...
  d_test_res <- tribble(
    ~day_nr, ~n_responses, ~n_new_cases, ~inc_cases,
    2, 1, NA, NA
  ) %>%
    mutate(
      n_new_cases = as.numeric(n_new_cases),
