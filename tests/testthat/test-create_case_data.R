...
d_ostrc <- tribble(
  ~id_participant, ~id_case, ~date_ostrc, ~q1, ~q2, ~q3, ~q4,
...
test_that("Returns correct dates.", {
  correct_startdate <- as.Date(c(
    "2023-01-01", "2022-12-07",
    "2023-01-12"
  ))

  correct_enddate <- as.Date(c(
    "2023-01-14", "2022-12-07",
    "2023-01-12"
  ))

  d_created <- create_case_data(
    d_ostrc, id_participant,
    id_case, date_ostrc,
    q1, q2, q3, q4
  )
...
test_that("Throws error if a health problem does not have a case id.", {
  ...
  expect_error(create_case_data(
    d_missing_caseid, id_participant,
    id_case, date_ostrc,
    q1, q2, q3, q4
  ))
})
...
test_that("will return dataframe without
  ...
  expect_warning(create_case_data(
    d_test, id_participant,
    id_case, date_ostrc,
    q1, q2, q3, q4
  ))
...
test_that("Returns warning if the first OSTRC Q has missing data.", {
  d_missing_q1 <- tribble(
    ~id_participant, ~id_case, ~date_ostrc, ~q1, ~q2, ~q3, ~q4,
    1, 1, "2023-01-01", NA, 0, 17, 25,
    1, 1, "2023-01-07", 8, 0, 17, 25
  )
  expect_warning(create_case_data(
    d_missing_q1, id_participant,
    id_case, date_ostrc,
    q1, q2, q3, q4
  ))
})
